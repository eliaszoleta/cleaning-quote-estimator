    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}>
      <form onSubmit={handleLogin} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 16, padding: 36, width: 340, boxShadow: '0 4px 24px rgba(0,0,0,0.07)' }}>
        ...
        <input
          ...
          style={{ ...inputStyle, marginBottom: 12, borderColor: pwError ? '#ef4444' : '#e2e8f0' }}
          autoFocus
        />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '32px 16px' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        ...
        {error && (
          <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '12px 16px', color: '#dc2626', fontSize: 13, marginBottom: 20 }}>{error}</div>
        )}

        {!supabase && (
          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '12px 16px', color: '#92400e', fontSize: 13, marginBottom: 20 }}>
            ...
            <form onSubmit={handleSave}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                ...
                <div>
                  ...
                  <input style={inputStyle} placeholder="(512) 555-0100" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
                </div>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, gridColumn: '1 / -1' }}>
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: form.active ? '#16a34a' : '#94a3b8', padding: 0 }}
                  >
                    {form.active ? <ToggleRight size={28} /> : <ToggleLeft size={28} />}
                  </button>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{form.active ? 'Will appear on results page' : 'Hidden from results page'}</span>
                </div>
              </div>
            ...
            {partners.map(p => (
              <div key={p.id} style={{ background: 'white', border: `1.5px solid ${p.active ? '#e2e8f0' : '#f1f5f9'}`, borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, opacity: p.active ? 1 : 0.6 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: '#64748b', marginTop: 2 }}>{p.city}, {p.state}{p.phone && ` · ${p.phone}`}{p.website && ` · ${p.website}`}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: p.active ? '#16a34a' : '#94a3b8', background: p.active ? '#f0fdf4' : '#f8fafc', border: `1px solid ${p.active ? '#bbf7d0' : '#e2e8f0'}`, borderRadius: 6, padding: '3px 9px' }}>
                    {p.active ? 'Active' : 'Inactive'}
                  </span>
                  <button onClick={() => handleToggle(p)} title={p.active ? 'Deactivate' : 'Activate'} style={{ background: 'none', border: 'none', cursor: 'pointer', color: p.active ? '#16a34a' : '#94a3b8', padding: 4 }}>
